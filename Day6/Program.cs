...
namespace Day6
{
    ...
    class Program
    {
        ...
        {
            List<(int id, Vector2i coord)> coords = ParseInput(input);
            var optimized = OptimizeOrigin(coords).ToList();

            //ProblemOne(optimized);
            ProblemTwo(optimized);
        }

        static void ProblemTwo(List<(int id, Vector2i coord)> coords){
            ...
            foreach (var x in Enumerable.Range(0, maxX)) {
                foreach (var y in Enumerable.Range(0, maxY)) {
                    ...
                    var distances = coords.Select(c => c.coord.DistanceTo(a)).Sum();
                    if (distances < 10000) {
                        area++;
                    }
                }                
        ...
        static void ProblemOne(List<(int id, Vector2i coord)> coords){
            int maxX = coords.Max(i => i.coord.X);
            int maxY = coords.Max(i => i.coord.Y);
            int[,] data = new int[maxX + 1, maxY + 1];
            ...
            foreach (var x in Enumerable.Range(0, maxX)) {
                foreach (var y in Enumerable.Range(0, maxY)) {
                    var a = new Vector2i(){X = x,Y = y};
                    var distances = coords.Select(c => (c.id, c.coord.DistanceTo(a))).OrderBy(i => i.Item2).ToList();
            ...
            Console.WriteLine($"{largest.index} {largest.area}");
        }
        static IEnumerable<(int id, Vector2i coord)> OptimizeOrigin(List<(int id,Vector2i coord)> coords)
        {
            Vector2i correction = new Vector2i(){X = coords.Min(i => i.coord.X), Y = coords.Min(i => i.coord.Y)};
            foreach(var tup in coords){
                yield return (tup.id, tup.coord-correction);
            }
        } 

        static List<(int id,Vector2i coord)> ParseInput(string input)
        {            
