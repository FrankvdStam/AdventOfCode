...
pub fn toggle(lights: &mut Vec<bool>, from: Vector2i, to: Vector2i)
{
    for i in from.x..to.x+1
    {
        ...
        {
            let val = lights[(i + (j * 1000)) as usize];
            lights[(i + (j * 1000)) as usize] = !val;
        }
    }
}
...
pub fn problem1()
{
    let mut lights: Vec<bool> = Vec::with_capacity(1000*1000);
    ...
    for line in INPUT.split("\n")
    {
        ...
        if chars[1] == 'u'
        {
            ...
            if chars[6] == 'f'
            {
                turn(&mut lights, from, to, false);
            }
            //on
            else
            {
                turn(&mut lights, from, to, true);
            }
        }
        //Toggle
        else
        {
            toggle(&mut lights, from, to);
        }
    }
...
static INPUT_: &'static str = "turn on 499,499 through 500,500";

static INPUT: &'static str = "toggle 461,550 through 564,900
