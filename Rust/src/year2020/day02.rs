...
pub fn problem1()
{
    ...
    for line in INPUT.split("\n").collect::<Vec<&str>>()
    {
        ...
        let char = line.chars().nth(space_index+1).unwrap();

        //We can skip this substring, just count the whole line -1
        //the min-max char: part of the format specifies the char once, hence we do count -1
        let occurrences = line.matches(char).count() - 1;
...
pub fn problem2()
{
    ...
    for line in INPUT.split("\n").collect::<Vec<&str>>()
    {
        ...
        let first = line.chars().nth((colon_index+1) + min).unwrap();
        let second = line.chars().nth((colon_index+1) + max).unwrap();

        //either char matches the input char but not both
...
static INPUT: &'static str = "5-10 b: bhbjlkbbbbbbb
3-4 j: hjvj
8-9 p: pmljtsttp
