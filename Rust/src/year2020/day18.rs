...
impl Token
{
    ...
    fn new_symbol(symbol: Symbol) -> Self
    {
        Token
        {
            is_number: false,
...
fn tokenize(input: &str) -> Vec<Token>
{
    ...
    //We can abuse the fact that the input does not contain numbers consisting of more than 1 character
    let no_whitespace_chars = input.chars().filter(|c| !c.is_whitespace()).collect::<String>().chars().collect::<Vec<char>>();
    for i in 0..no_whitespace_chars.len()
    {
        match no_whitespace_chars[i]
        {
            '(' => tokens.push(Token::new_symbol(Symbol::ParenthesisOpen)),
            ')' => tokens.push(Token::new_symbol(Symbol::ParenthesisClose)),
            '+' => tokens.push(Token::new_symbol(Symbol::Plus)),
            '*' => tokens.push(Token::new_symbol(Symbol::Asterisk)),
            _ => tokens.push(Token::new_number( no_whitespace_chars[i].to_string().parse::<i64>().unwrap())),
        }
    }
...
static INPUT: &'static str = "2 + (2 + 6 * (4 * 5 * 8 * 7) + 3) * (9 + (4 * 3 * 7 * 9)) * 7
