...
pub fn problem1()
{
    ...
    for i in 0..program.len()
    {
        ...
        match program[i].is_mask
        {
            true => {
                mask = &program[i].mask;
            }
            false => {
                let memory_value = memory.entry(program[i].memory_location).or_insert_with(|| 0);
                *memory_value = apply_mask(mask, program[i].value);
            }
        }
    }

    let sum: usize = memory.iter().map(|p| p.1).sum();
    println!("{}",sum);
}
...
pub fn problem2()
{
    ...
    //let mask2 = apply_mask_part2(&program[0].mask, program[1].memory_location);
    //println!("{:?}", mask2);
    //println!("{:?}", program[0].mask);
    //println!("{:?}", get_floating(&program[0].mask, 0));
//
    //return;
    ...
    for i in 0..program.len()
    {
        if program[i].is_mask
        {
            mask = &program[i].mask;
            //masks = get_masks(mask, 0);
        }
        else
        {
            let mem_mask = apply_mask_part2(mask, program[i].memory_location);
            let masks = get_floating(&mem_mask, 0);
            //println!("{}", masks.len());
            for j in 0..masks.len()
            {
                let memory_value = memory.entry(masks[j]).or_insert_with(|| 0);
                *memory_value = program[i].value;
                //println!("{:?}", masks);
            }
           ...
           // for j in 0..masks.len()
           // {
           //     let memory_location = apply_mask(&masks[j], program[i].memory_location);
           //     println!("[{}] = {}", memory_location, program[i].value);
//
           //     let memory_value = memory.entry(memory_location).or_insert_with(|| 0);
           //     *memory_value = program[i].value;
           // }
        }
    }
...
fn get_floating(mask: &Vec<char>, bit: usize) -> Vec<usize>
{
    return if bit >= 36
    {
        ...
        let mut resultvec: Vec<usize> = Vec::new();
    ...
    else
    {
        if mask[bit] == 'X'
        {
            ...
            zero[bit] = '0';
            let mut one = mask.clone();
            one[bit] = '1';

            let zeros = get_floating(&zero, bit + 1);
            let ones = get_floating(&one, bit + 1);
        ...
        {
            get_floating(mask, bit + 1)
        }
    }
}


fn apply_mask_part2(mask: &Vec<char>, value: usize) -> Vec<char>
{
    ...
    {
        //Check if bit is set
        if value & (1usize << (35usize -i as usize)) > 0 && mask[i as usize] != 'X'
        {
            new_mask[i as usize] = '1';
        }
        i -= 1;
    }
    return new_mask;
...
fn apply_mask(mask: &Vec<char>, mut value: usize) -> usize
{
    //println!("applying mask to {}", value);
    let mut i: i64 = 35;
    while i >= 0
    {
        //println!("i: {}", i);
        match mask[i as usize]
        {
            '0' => {
                //clear bit at i
                let mut temp_mask: usize = 0xffffffffffffffff;
                temp_mask ^= 1usize << (35 - i);
                //println!("temp mask: {}", format!("{:b}", temp_mask));
            ...
            '1' => {
                //set bit at i
                value |= 1usize << (35 - i);
                //println!("set bit at {} {}",i, format!("{:b}", value));
            }
            _ => {}
        }
        i -= 1;
    ...
    //Deal with overflow
    let overflow = 0x000fffffffffffff;
    if value > overflow
    {
        println!("overflow: {}",value);
    }
...
impl Instruction
{
    pub fn new_mask(mask: Vec<char>) -> Self
    {
        Instruction
        {
            is_mask: true,
    ...
    pub fn new_memory_set(memory_location: usize, value: usize) -> Self
    {
        Instruction
        {
            is_mask: false,
    ...
    pub fn to_string(&self) -> String
    {
        if self.is_mask
        {
            return self.mask.iter().map(|&id| id.to_string() + ",").collect();
        }
        else
        {
            return format!("[{}] = {}", self.memory_location, self.value);
        }
    }
...
fn parse_program(input: &str) -> Vec<Instruction>
{
    ...
    for i in 0..split.len()
    {
        ...
        if split[i].starts_with("mask")
        {
            //Get the mask
            let mask = split[i][7..].chars().collect::<Vec<char>>();
            instruction = Instruction::new_mask(mask);
        }
        else
        {
            //location always starts at 4
            let end_index = split[i].find(']').unwrap();
            let memory_location = split[i][4..end_index].parse::<usize>().unwrap();

            let start_index = split[i].find('=').unwrap()+2;
            let value = split[i][start_index..].parse::<usize>().unwrap();

            instruction = Instruction::new_memory_set(memory_location,value);
        }
...
static EXAMPLE2: &'static str = "mask = 000000000000000000000000000000X1001X
mem[42] = 100
mask = 00000000000000000000000000000000X0XX
mem[26] = 1";

static EXAMPLE1: &'static str = "mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X
mem[8] = 11
mem[7] = 101
mem[8] = 0";


static INPUT: &'static str = "mask = 010X1100101X00X01001X11010X111100X01
mem[23014] = 9778
mem[42882] = 140716
mem[65461] = 458355100
mem[60151] = 31172
