...
pub fn problem1()
{
    ...
    {
        if credentials[i].is_valid_passport()
        {
            count += 1;
        }
    }
...
pub fn problem2()
{
    ...
    for i in 0..credentials.len()
    {
        if credentials[i].strict_is_valid_passport()
        {
            count += 1;
        }
    }
...
impl Credentials
{
    ...
    fn from_str(str: &str) -> Self
    {
        ...
        for i in 0..split.len()
        {
            if split[i] != ""
            {
                let split2 = split[i].split(':').collect::<Vec<&str>>();
                let field = split2[0];
                let value = split2[1];
                ...
                {
                    "byr" => {
                        credentials.byr = Some(value.parse::<usize>().unwrap())
                    },
                    "iyr" => {
                        credentials.iyr = Some(value.parse::<usize>().unwrap())
                    },
                    "eyr" => {
                        credentials.eyr = Some(value.parse::<usize>().unwrap())
                    },
                    "hgt" => {
                        credentials.hgt = Some(value.to_string())
                    },
                    "hcl" => {
                        credentials.hcl = Some(value.to_string())
                    },
                    "ecl" => {
                        credentials.ecl = Some(value.to_string())
                    },
                    "pid" => {
                        credentials.pid = Some(value.to_string())
                    },
                    "cid" => {
                        credentials.cid = Some(value.parse::<usize>().unwrap())
                    },
    ...
    fn strict_is_valid_passport(&self) -> bool
    {
        if !self.is_valid_passport()
        {
            return false;
        }
        //We now know all fields are some and we can safely unwrap all fields (except cid)
        ...
        let heightstr = self.hgt.as_ref().unwrap().clone();
        let height;
        match heightstr[..heightstr.len()-2].parse::<usize>(){
            Ok(v) => height = v,
            Err(_) => return false
        };
        ...
        //#c0946f
        if self.hcl.as_ref().unwrap().len() != 7
        {
            return false;
        }
        else
        {
            let mut chars = self.hcl.as_ref().unwrap().chars();
        ...
        match self.pid.as_ref().unwrap().parse::<i64>()
        {
            Err(_) => return false,
            _ => {}
        }
...
fn parse_credentials(input: &str) -> Vec<Credentials>
{
    ...
    for i in 0..split.len()
    {
        if split[i] == "" || i >= split.len()-1
        {
            ...
            {
                str.push_str(split[j]);
                str.push(' ');
            }

            let cred = Credentials::from_str(str.as_str());
...
static EXAMPLE_INVALID: &'static str = "eyr:1972 cid:100
...
static EXAMPLE_VALID: &'static str = "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
static EXAMPLE: &'static str = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
static INPUT: &'static str = "eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
