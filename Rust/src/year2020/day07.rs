...
pub fn problem1()
{
    ...
    let mut name_index_lookup = HashMap::new();
    parse_bags(INPUT, &mut bags, &mut name_index_lookup);
    //print_bags(&bags, &name_index_lookup);

    let parents = find_unique_parents(&bags, name_index_lookup["shiny gold"]);
    println!("{}", parents.len());
}
...
pub fn problem2()
{
    ...
    let mut index_name_lookup: HashMap<usize, String> = HashMap::new();
    for item in name_index_lookup.iter()
    {
        index_name_lookup.insert(item.1.clone(), item.0.clone());
    }

    //-1 because it includes the shiny gold bag itself
    let count = count_bags_recursive(&bags, name_index_lookup["shiny gold"])-1;
    println!("{}", count);
}

fn find_unique_parents(bags: &Vec<Bag>, child_index: usize) -> Vec<usize>
{
    let mut parents = Vec::new();

    //Copy all parents into a stack objects
    let mut stack = bags[child_index].parents.clone();

    while stack.len() > 0
    {
        let parent_index = stack.pop().unwrap();
        if !parents.contains(&parent_index)
        {
            parents.push(parent_index);
        }

        for i in 0..bags[parent_index].parents.len()
        {
            stack.push(bags[parent_index].parents[i]);
        }
    }
...
fn print_bags(bags: &Vec<Bag>, name_index_lookup: &HashMap<String, usize>)
{
    //Reverse hashmap into index name lookup
    let mut index_name_lookup = HashMap::new();
    for item in name_index_lookup.iter()
    {
        index_name_lookup.insert(item.1, item.0);
    }

    for b in bags.iter()
    {
        print!("{} - ", index_name_lookup[&b.index]);
        for i in 0..b.children.len()
        {
            let (index, quantity) = b.children[i];
            print!("{}x {} ", quantity, index_name_lookup[&index]);
        }
        println!();
    }
}
...
fn parse_bags(input: &str, bags: &mut Vec<Bag>, name_index_lookup: &mut HashMap<String, usize>)
{
    let split = input.split('\n').collect::<Vec<&str>>();

    //First pass: create objects with name and index
    for i in 0..split.len()
    {
        let bits = split[i].split(' ').collect::<Vec<&str>>();
        let mut name = String::from(bits[0]);
        name.push(' ');
        name.push_str(bits[1]);

        bags.push(Bag::new(name.clone(), i));

        //Also add this name and index to a lookup
        name_index_lookup.insert(name.clone(), i);
    }

    //println!("{:?}", name_index_lookup);


    //2nd pass: add children and quantities
    for parent_index in 0..split.len()
    {
        let bits = split[parent_index].split(' ').collect::<Vec<&str>>();

        let mut bits_index = 4;
        'child_loop: while bits_index < bits.len()
        {
            if bits[4] == "no"
            {
                break 'child_loop;
            ...
            //parse
            let quantity = bits[bits_index].parse::<usize>().unwrap();
            let mut name = String::from(bits[bits_index+1]);
            name.push(' ');
            name.push_str(bits[bits_index+2]);

            //Find the bag object that matches with this parent object
            //println!("{}", name);
            let child_index = name_index_lookup[&name];

            //Set parent and child relation
            bags[parent_index].children.push((child_index, quantity));
            bags[child_index].parents.push(parent_index);
            bits_index += 4;
        }
    }
}
...
fn count_bags_recursive(bags: &Vec<Bag>, index: usize) -> usize
{
    //println!("arrived at {} with multiplication {}", index_name_lookup[&index], multiplication);

    //If there are no children, we are just 1 bag
    if bags[index].children.len() == 0
    {
        return 1;
    }

    let mut count = 0;
    for i in 0..bags[index].children.len()
    {
        let recursive_count = count_bags_recursive(&bags, bags[index].children[i].0);
        count += bags[index].children[i].1 * recursive_count;
    }
    return count + 1;
}


static CUSTOM_EXAMPLE: &'static str = "shiny gold bags contain 2 dark olive bag, 2 vibrant plum bags
dark olive bags contain 2 vibrant plum bags.
vibrant plum bags contain no other bags.";

static EXAMPLE1: &'static str = "light red bags contain 1 bright white bag, 2 muted yellow bags.
...
static EXAMPLE2: &'static str = "shiny gold bags contain 2 dark red bags.
...
static INPUT: &'static str = "striped green bags contain 5 posh indigo bags.
