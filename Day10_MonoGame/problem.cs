...
namespace Day10_MonoGame
{
    public class Vector2i
    {
        ...
        {
            return new Vector2i() { X = left.X - right.X, Y = left.Y - right.Y };
        }
        public static Vector2i operator +(Vector2i left, Vector2i right)
        {
            return new Vector2i() { X = left.X + right.X, Y = left.Y + right.Y };
    ...
    public class problem
    {
        ...
        public static int FindBiggestDistance(List<Light> lights)
        {
            ...
            foreach (Light l1 in lights)
            {
                foreach (Light l2 in lights)
                {
                    ...
                    int dist = l1.Position.DistanceTo(l2.Position);
                    if (dist > biggest)
                    {
                        biggest = dist;
                    }
        ...
        public static List<Light> ParseInput(bool useExampleData)
        {
            if (useExampleData)
            {
                return ParseInput(example, 2, 2);
            }
            else
            {
                return ParseInput(input, 6, 2);
            }
        }

        public static void ProgresSeconds(int seconds, List<Light> lights)
        {
        ...
        public static List<Light> MoveToOrigin(List<Light> lights)
        {
            ...
            foreach (Light l in lights)
            {
                result.Add(
                    new Light()
                    {
                        Position = l.Position + correction,
                        Velocity = l.Velocity,
                    }
                );
            }
        ...
        public static List<Light> Normalize(List<Light> lights)
        {
            var result = new List<Light>();
            Vector2i correction = new Vector2i()
            {
        ...
        public static void Draw(List<Light> lights)
        {
            Console.Clear();
            foreach (Light l in lights)
            {
        ...
        public static List<Light> ParseInput(string input, int positionSize, int velocitySize)
        {
            var split = input.Split(new string[] { "\r\n" }, StringSplitOptions.None).ToList();
            //The numbers are always in the same place, with slight variance in their size.
            int positionStartIndex = 10;
            int secondPositionStartIndex = positionStartIndex + positionSize + 1 /*comma*/ + 1 /*whitespace*/;
            int velocityStartIndex = positionStartIndex + positionSize + 1/*comma*/ + 1 /*whitespace*/ + positionSize + 12;
            int secondVelocityStartIndex = velocityStartIndex + velocitySize + 1/*comma*/ + 1 /*whitespace*/;

            List<Light> lights = new List<Light>();
            foreach (string s in split)
            {
                //var X = int.Parse(s.Substring(velocityStartIndex, velocitySize).Trim(' '));
                //var Y = int.Parse(s.Substring(secondVelocityStartIndex, velocitySize).Trim(' '));

                lights.Add(new Light()
                {
                    Position = new Vector2i()
                    {
                        X = int.Parse(s.Substring(positionStartIndex, positionSize).Trim(' ')),
                        Y = int.Parse(s.Substring(secondPositionStartIndex, positionSize).Trim(' ')),
                    },
                    Velocity = new Vector2i()
                    {
                        X = int.Parse(s.Substring(velocityStartIndex, velocitySize).Trim(' ')),
                        Y = int.Parse(s.Substring(secondVelocityStartIndex, velocitySize).Trim(' ')),
                    }
                });
            }
            return lights;
        ...
        public static string example = @"position=< 9,  1> velocity=< 0,  2>
...
position=<-3,  6> velocity=< 2, -1>";
        ...
        public static string input = @"position=< 20316, -30055> velocity=<-2,  3>
