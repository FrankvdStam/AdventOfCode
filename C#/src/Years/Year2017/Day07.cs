...
namespace Years.Year2017
{
    ...
    public class Day07 : IDay
    {
        ...
        {
            var node = FindOffendingNode(ParseInput(Input));
            var parent = node.Parent;

            var weights = parent.SubNodes.Select(i => i.GetTowerWeight()).ToList();
            var weightOccurrences = new Dictionary<int, int>();
            foreach (var w in weights)
            {
                if (weightOccurrences.ContainsKey(w))
                {
                    weightOccurrences[w]++;
                }
                else
                {
                    weightOccurrences[w] = 1;
                }
            }

            var correctTotalWeight = weightOccurrences.Single(i => i.Key != node.GetTowerWeight()).Key;
            var towerWeight = node.GetTowerWeight();
            var difference = correctTotalWeight - towerWeight;
            var result = node.Weight + difference;
            Console.WriteLine(result);
        }

        private Node FindOffendingNode(Node n)
        {
            ...
            while (stack.Any())
            {
                ...
                var weights = node.SubNodes.Select(i => i.GetTowerWeight()).ToList();
                var weightOccurrences = new Dictionary<int, int>();
                foreach (var w in weights)
                {
                    if (weightOccurrences.ContainsKey(w))
                ...
                //Check if all childs have the same weight
                if (weightOccurrences.Count > 1)
                {
                    var index = weights.IndexOf(weightOccurrences.Single(i => i.Value == 1).Key);
                    var offendingNode = node.SubNodes[index];
                    stack.Push(offendingNode);
                }
                else
                {
                    return node;
        ...
        private void WriteNode(Node n, int height)
        {
            int x = Console.CursorLeft;
            int y = Console.CursorTop;

            Console.Write($" {n.Name} {n.GetTowerWeight()} ");

            if (n.SubNodes.Any())
            {
                Console.Write("-");
                int tempY = Console.CursorLeft;
                int tempX = Console.CursorTop;
                for (int i = 0; i < n.SubNodes.Count; i++)
                {
                    //Connect the line
                    if (tempX + 1 != Console.CursorTop)
                    {
                        int target = Console.CursorTop;
                        while (tempX < target)
                        {
                            Console.CursorTop = tempX;
                            Console.Write('|');
                            Console.CursorLeft = tempY;
                            tempX++;
                        }
                    }
                    tempX++;

                    if (i == 0 && n.SubNodes.Count < 2)
                    {
                        Console.Write('-');
                    }
                    else if (i + 1 == n.SubNodes.Count)
                    {
                        Console.Write('\\');
                    }
                    else
        ...
        {
            //For readabilities sake I'm going to do 2 passes
            Dictionary<string, Node> lookup = new Dictionary<string, Node>();

            //First pass: create node object for each node and add them to a lookup
            var lines = input.Split(new string[] { "\r\n" }, StringSplitOptions.None);
            foreach (var line in lines)
            {
                var bits = line.Split(' ');

                string name = bits[0].Replace(",", string.Empty);
                int weight = int.Parse(bits[1].Replace("(", string.Empty).Replace(")", string.Empty));

                lookup.Add(name, new Node
                {
                    Name = name,
                    Weight = weight
                });
            ...
            //Second pass: set parent and child relations
            foreach (var line in lines)
            {
                var bits = line.Split(' ');

                string name = bits[0].Replace(",", string.Empty);
                Node parentNode = lookup[name];

                if (bits.Length > 3)
                {
                    for (int i = 3; i < bits.Length; i++)
                    {
                        string childName = bits[i].Replace(",", string.Empty);
                        Node childNode = lookup[childName];
        ...
        private const string Input = @"ifyzcgi (14)
axjvvur (50)
...
swnafht (44) -> lbxdtms, jensj, zkpfzio, jobwpao, jxfimbl";
        ...
        private const string Example = @"pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
