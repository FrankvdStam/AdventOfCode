...
namespace Day04
{
    class Program
    {
        ...
        {
            //var sentences = ParseInput(Example);
            var sentences = ParseInput(Input);
            ProblemTwo(sentences);
            //ProblemTwo();
        }

        static void ProblemOne(List<List<string>> sentences)
        {
            int count = 0;
        ...
        static void ProblemTwo(List<List<string>> sentences)
        {
            int count = 0;
            foreach (var sentence in sentences)
            {
                ...
                foreach (var word in sentence)
                {
                    if (sentence.Count(i => i == word) > 1 || sentence.Any(j => j != word && CanRearrange(word, j)))
                    {
                        isValid = false;
                        break;
                    }
        ...
        static bool CanRearrange(string a, string b)
        {
        ...
        private static string Input = @"nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
inc mpys mzqmcwx vryz ibqrzc pmsy fat rojpxwy rcbqzi gjef";
        ...
        private static string Example = @"abcde fghij
abcde xyz ecdab
a ab abc abd abf abj
iiii oiii ooii oooi oooo
oiii ioii iioi iiio";
/*
        private static string Example = @"aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa";
*/
    }
