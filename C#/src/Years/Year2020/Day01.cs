    ...
    public class Day01 : IDay
    {
        ...
        public void ProblemTwo()
        {
            for (int i = 0; i < _numbers.Count; i++)
            {
                for (int j = i + 1; j < _numbers.Count; j++)
                {
                    for (int z = j + 1; z < _numbers.Count; z++)
                    {
                        if (_numbers[i] + _numbers[j] + _numbers[z] == 2020)
                        {
                            Console.WriteLine(_numbers[i] * _numbers[j] * _numbers[z]);
                            return;
                        }
                    }
                }
        ...
        private const string Input = @"1732
1972
1822
1920
1847
