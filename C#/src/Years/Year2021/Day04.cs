...
namespace Years.Year2021
{
    ...
    public class Day04 : IDay
    {
        ...
        public void ProblemOne()
        {
            ...
            for (int i = 0; i < drawNumbers.Count; i++)
            {
                ...
                foreach (var card in cards)
                {
                    ...
                    {
                        var score = card.CalculateScore(number);
        ...
        public List<BingoCard> ParseInput(string input, out List<int> drawNumbers)
        {
            var lines = input.SplitNewLine();
            
            drawNumbers = new List<int>();
            var drawNumbersStr = lines[0];
            foreach (var num in drawNumbersStr.Split(','))
            {
                drawNumbers.Add(int.Parse(num));
            }

            var result = new List<BingoCard>();
            for (int i = 2; i < lines.Length; i += 6)
            {
                var bingoData = new int[5, 5];
                for (int y = 0; y < 5; y++)
                {
                    var bits = lines[i + y];
                    var split = bits.Split(' ', StringSplitOptions.RemoveEmptyEntries);

                    for (int x = 0; x < 5; x++)
                    {
                        bingoData[x, y] = int.Parse(split[x]);
                    }
                }
                result.Add(new BingoCard(bingoData));
            }
