...
namespace Years.Year2021
{
    public class Day07 : IDay
    {
        ...
        public void ProblemOne()
        {
            ...
            {
                var cost = FindFuelCost(positions, i);
                results[i] = cost;
            }

            var best = results.Min(i => i.Value);
        ...
        public void ProblemTwo()
        {
            ...
            for (int i = min; i <= max; i++)
            {
                var cost = FindFuelCostIncremental(positions, i);
                results[i] = cost;
            }
        ...
        private int FindFuelCost(List<int> positions, int alignmentPosition)
        {
            var cost = 0;
            foreach (var p in positions)
            {
                cost += Math.Abs(p - alignmentPosition);
            }
            return cost;
        }


        private int FindFuelCostIncremental(List<int> positions, int alignmentPosition)
        {
            var cost = 0;
            foreach (var p in positions)
            {
                var distance = Math.Abs(p - alignmentPosition);
                cost += AdditionFactorial(distance);
            }
            return cost;
        }


        private Dictionary<int, int> _factorials = new Dictionary<int, int>();

        private int AdditionFactorial(int num)
        {
