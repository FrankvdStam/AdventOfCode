...
namespace Years.Year2016
{
    public class Day04 : IDay
    {
        ...
        public void ProblemTwo()
        {
            ...
            foreach (var line in lines)
            {
                roomNames.Add(Decode(line));
            }

            var result = roomNames.Where(i => i.Contains("north"));
            var bits = result.First().Split(' ');
            var roomNumber = bits[bits.Length - 1];
        ...
        private string Decode(string input)
        {
            ...
            int sectorId = int.Parse(input.Substring(startIndex, input.IndexOf('[') - startIndex));

            int shifts = sectorId % 26;//No need to shift 300 times when there are just 26 states


            string decoded = "";
            foreach (char c in input.Substring(0, startIndex))
            {
                if (c == '-')
                {
                    decoded += ' ';
        ...
        private char Shift(char c, int shifts)
        {
            ...
            {
                int diff = (('z' + 1) - c);
                int shiftsFromA = shifts - diff;

                return (char)('a' + shiftsFromA);
            }
            else
            {
                char a = (char)(c + shifts);

                return (char)(c + shifts);
            }
        ...
        private bool IsReal(string room, out int sectorId)
        {
            sectorId = 0;
            ...
            {
                if (char.IsNumber(room[i]))
                {
                    firstNumberIndex = i;
                    break;//Continue with parsing
                }
                else
                {
                    if (room[i] != '-')
            ...
            int numberEndIndex = room.IndexOf('[');
            string num = room.Substring(firstNumberIndex, numberEndIndex - firstNumberIndex);
            sectorId = int.Parse(num);

            string checksum = room.Substring(numberEndIndex + 1, room.Length - numberEndIndex - 2);
            ...
            {
                count[c] = chars.Count(i => i == c);
            }

            List<KeyValuePair<char, int>> countList = count.ToList();
            ...
            while (commited < 5)
            {
                var keyvalues = countList.Where(i => i.Value == countList.Max(j => j.Value)).ToList();
                keyvalues.Sort(
                    delegate (KeyValuePair<char, int> pair1,
                        KeyValuePair<char, int> pair2)
                    {
                ...
                countList.Remove(keyvalue);
                generatedChecksum += keyvalue.Key;
                commited++;
            }
        ...
        private const string Input = @"bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
hplazytkpo-prr-cpnptgtyr-379[prtya]";
        ...
        private const string Example = @"aaaaa-bbb-z-y-x-123[abxyz]
