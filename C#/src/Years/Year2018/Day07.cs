...
namespace Years.Year2018
{
    public class Day07 : IDay
    {
        ...
        private int baseStepDuration = 60;
        private int GetStepDuration(string stepName)
        {
            return baseStepDuration + (stepName[0] - 'A') + 1;
        }

        private void RemoveStepFromDictionary(Step step, Dictionary<string, Step> steps)
        {
        ...
        private Step FindNextStep(Dictionary<string, Step> steps)
        {
            return steps.Values.Where(i => !i.DependsOn.Any() && !i.InProgress).OrderBy(i => i.StepName).FirstOrDefault();
        }

        //input: Step C must be finished before step A can begin.
        private Dictionary<string, Step> ParseInput(string input)
        {
