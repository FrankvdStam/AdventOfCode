...
namespace Years.Year2015
{
    class Program8
    {
        static void Main2()
        {
            var lines = File.ReadLines(Environment.CurrentDirectory + @"\file.txt");
        ...
        {
            var code = ReadFromFile("file.txt");
            var memory = escapedInput;

            var codeCount = code.Sum(i => i.Length);
            var memoryCount = memory.Sum(i => i.Length);
            var result = codeCount - memoryCount;
        }
        ...
        static List<string> ReadFromFile(string filename)
        {
            ...
            using (FileStream fileStream = new FileStream(Environment.CurrentDirectory + $@"\{filename}", FileMode.Open, FileAccess.Read))
            {
        ...
        private static string[] escapedExample = { "", "abc", "aaa\"aaa", "\x27" };

        static string[] escapedInput = {"sjdivfriyaaqa\xd2v\"k\"mpcu\"yyu\"en"  ,
"vcqc"                                                                  ,
