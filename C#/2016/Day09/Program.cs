...
namespace Day09
{
    class Program
    {
        ...
        {
            //ProblemOne("X(8x2)(3x3)ABCY");
            ProblemTwoThrirdTry(Input);
            //ProblemTwo(Input);
        }

        static string ProblemOne(string input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                if (input[i] == '(')
                {
                    ...
                    string marker = input.Substring(i + 1, j - i - 1);
                    var split = marker.Split('x');
                    int chars = int.Parse(split[0]);
                    int repeat = int.Parse(split[1]);

                    string repeatString = input.Substring(j + 1, chars);
        ...
        static void ProblemTwo(string input)
        {
            ...
            while (input.Contains('(') || input.Contains(')'))
            {
                ...
                //Remove all innocent chars from the string and count these chars instead
                input = input.Substring(startIndex);
                count += startIndex;
               
                //That means input now starts at a marker. Deal with this marker.
                int endIndex = input.IndexOf(')');
                
                string marker = input.Substring(1 /*1 because we skip the ( at 0*/, endIndex-1);
                var split = marker.Split('x');
                int chars = int.Parse(split[0]);
                int repeat = int.Parse(split[1]);
                ...
                string appendString = input.Substring(endIndex+1, chars);
                ...
                str.Append(input.Substring(endIndex+1 + chars));
                input = str.ToString();

                if (cycle % 10000 == 0)
                {
                    Console.WriteLine(cycle + ": " +  input.Length);
                }
                cycle++;
            }
            
            //Remaining chars have to still be added to count
            count += input.Length;
        }
        //https://www.reddit.com/r/adventofcode/comments/5hbygy/2016_day_9_solutions/
        //Nice idea by rhardih
        static void ProblemTwoThrirdTry(string input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                if (input[i] == '(')
                {
                    ...
                    //Increase weights
                    for (int k = j+1; k < j+1 + chars; k++)
                    {
                        weights[k] *= repeat;
                    }

                    //Continue on beyond the marker
                    i = j;
                }
                else
                {
