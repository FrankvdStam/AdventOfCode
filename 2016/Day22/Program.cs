...
namespace Day22
{
    ...
    {
        public static IEnumerable<(int x, int y, T t)> IterateMultiArray<T>(this T[,] array)
        {
            for (int x = 0; x < array.GetLength(0); x++)
            {
                for (int y = 0; y < array.GetLength(1); y++)
                {
                    if (array[x, y] == null)
                    {
                        throw new Exception($"Element null at ({x}, {y})");
                    }
    ...
    class Program
    {
        ...
        {
            //var nodes = ParseInput(Input, 35, 29);
            var nodes = ParseInput(Input, 35, 29);
        ...
        static void ProblemOne(Node[,] nodes)
        {
            List<string> keys = new List<string>();
            foreach (var inner in nodes.IterateMultiArray())
            {
                foreach (var outer in nodes.IterateMultiArray())
                {
                    ...
                    string key1 = $"({inner.x}, {inner.y}), ({outer.x}, {outer.y})";
                    string key2 = $"({outer.x}, {outer.y}), ({inner.x}, {inner.y})";

                    if (IsNodePairViable(inner.t, outer.t) && !keys.Contains(key1) && !keys.Contains(key2))
                    {
                        keys.Add(key1);
                    }
                }
        ...
        static bool IsNodePairViable(Node a, Node b)
        {
            return a.Used > 0 && a != b && b.Available > a.Used;
        }


        #region sliddy puzzle
        //The plan is not the store a whole board for each step
        //I will search breadth first, storing the whole map will explode my memory

        //Globally for each state we have:
        //  -size (width, height)
        //  -Unmovable points or walls
        //  -Accessable point or endgoal

        //Each state needs to keep track of a number of points:
        //  -Empty point
        //  -List of moves to get to this point
        //  -Point where the payload is that needs to be delivered to the end location
        static void ProblemTwo(Node[,] nodes)
        {
            //Generate all the data we need for the search

            //Get the initial state
            var emptyNode = nodes.IterateMultiArray().FirstOrDefault(i => i.t.Used == 0);
            Vector2i empty = new Vector2i(emptyNode.x, emptyNode.y);
            Vector2i payload = new Vector2i(nodes.GetLength(0) - 1, 0);

            State initialState = new State();
            initialState.Empty = empty;
            initialState.Payload = payload;
            int unmovableBoundary = 500;

            //Get unmovable points
            List<Vector2i> unmovable = new List<Vector2i>();
            foreach (var n in nodes.IterateMultiArray().Where(i => i.t.Size > unmovableBoundary))
            {
                unmovable.Add(new Vector2i(n.x, n.y));
            }

            Vector2i goal = new Vector2i(0, 0);
            int width  = nodes.GetLength(0);
            int height = nodes.GetLength(1);
            int step = 0;
            
            DrawState(initialState, goal, unmovable, width, height);
            ...
            while (queue.Any())
            {
                ...
                Console.WriteLine($"Step {step}: resolving {states.Count} states.");

                foreach (var s in states)
                {
                    var moves = GetPotentialMoves(s, unmovable, width, height);
        ...
        static List<Vector2i> GetPotentialMoves(State state, List<Vector2i> unmovable, int width, int height)
        {
            List<Vector2i> moves = new List<Vector2i>();

            for (int i = 0; i + 1 < directions.Length; i += 2)
            {
                int x = state.Empty.X + directions[ i];
                int y = state.Empty.Y + directions[i + 1];

                if (x >= 0 && x < width && y >= 0 && y < height)
                {
                    Vector2i potentialMove = new Vector2i(x, y);
                    if (!unmovable.Contains(potentialMove))
                    {
                        //Try to prevent repeating moves
                        if (state.Moves.Count > 0 && state.Moves.Last().First == potentialMove)
                        {
                            continue;
                        }
        ...
        static void DrawState(State s, Vector2i goal, List<Vector2i> unmovable, int width, int height)
        {
            //clear and draw dots everywhere
            Console.Clear();
            ...
            //Draw the unmovable's
            foreach (var vec in unmovable)
            {
            ...
            //Draw the state in
            Console.SetCursorPosition(s.Empty.X, s.Empty.Y);
            Console.Write('_');
            Console.SetCursorPosition(s.Payload.X, s.Payload.Y);
            Console.Write('P');
            ...
            //Set the cursor outside of the view
        ...
        public class State
        {
            public State Copy()
            {
                return new State(){Empty = Empty.Copy(), Payload = Payload.Copy(), Moves = new List<(Vector2i First, Vector2i Second)>(Moves)};
            }

            public Vector2i Empty;
            public Vector2i Payload;
            public List<(Vector2i First, Vector2i Second)> Moves = new List<(Vector2i First, Vector2i Second)>();
            
            public void DoMove(Vector2i move)
            {
                //Add the move
                Moves.Add((Empty.Copy(), move.Copy()));

                //If we just moved empty to the location of the payload, the payload has to move to the location of empty.
                if (move == Payload)
                {
                    Payload = Empty.Copy();
                }
                //Finally move empty to the move location
                Empty = move.Copy();
            }
            ...
            public override string ToString()
            {
                return $"Empty: {Empty} Payload: {Payload} Moves: {Moves.Count}";
            }
        }
        

        #endregion
        static void DrawNodeMap(Node[,] nodes)
        {
            //Opted not to set cursorposition with IterateMultiArray
            //instead loop x in inner so that the chars can be writen without
            //changing the cursor pos, should make drawing the map a bunch faster.
            ...
            Console.SetCursorPosition(nodes.GetLength(0)-1, 0);
            Console.Write('G');
        ...
        static Node[,] ParseInput(string input, int width, int height)
        {
            ...
            {
                var bits = line.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);

                //Parse the coords
                var coords = bits[0].Split('-');
                int x = int.Parse(coords[1].Replace("x", String.Empty));
                int y = int.Parse(coords[2].Replace("y", String.Empty));

                //Parse the sizes
                int size           = int.Parse(bits[1].Replace("T", String.Empty));
                int used           = int.Parse(bits[2].Replace("T", String.Empty));
                int available      = int.Parse(bits[3].Replace("T", String.Empty));
                int usedPercentage = int.Parse(bits[4].Replace("%", String.Empty));

                nodes[x, y] = new Node(size, used, available, usedPercentage);
            }
        ...
        public class Node
        {
            public Node(int size, int used, int available, int usedPercentage)
            {
                Size = size;
                Used = used;
                Available = available;
            ...
            public override string ToString()
            {
                return $"{Size}: {Used}/{Available} - {UsedPercentage}";
            }
        }
        ...
        private static string Example = @"/dev/grid/node-x0-y0   10T    8T     2T   80%
...
/dev/grid/node-x2-y2    9T    6T     3T   66%";
        ...
        private static string Input = @"/dev/grid/node-x0-y0     89T   65T    24T   73%
