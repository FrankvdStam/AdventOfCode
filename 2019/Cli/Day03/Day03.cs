...
namespace Cli.Day03
{
    public class Day03 : IDay
    {
        ...
        {
            ParseInput(Example, out _, out List<Vector2i> intersections);
            List<int> distances = new List<int>();
            Vector2i zero = new Vector2i();
            foreach (var intersection in intersections)
            {
                distances.Add(intersection.ManhattanDistance(zero));
            }
        ...
        //private void FindIntersections(List<List<Vector2i>> wires, out List<Vector2i> intersections)
        //{
        //    //Keep track of all the coordinates we've been - this is not a memory effecient solution
        //    List<Vector2i> coords =  new List<Vector2i>();
        //    intersections = new List<Vector2i>();
        //
        //    foreach (var wire in wires)
        //    {
        //        Vector2i position = new Vector2i();
        //        foreach(var vec in wire)
        //        {
        //            var line = GenerateLine(position, vec);
        //            foreach (var l in line)
        //            {
        //                if (coords.Contains(l))
        //                {
        //                    intersections.Add(l);
        //                }
        //                else
        ...
        //}
        //
        private List<Vector2i> GenerateLine(Vector2i start, Vector2i end)
        {
            List<Vector2i> line = new List<Vector2i>();
            if (start.X == end.X)
            {
                int min = Math.Min(start.Y, end.Y)+1;
                int max = Math.Max(start.Y, end.Y);
        
                for (int i = min; i <= max; i++)
                {
                    line.Add(new Vector2i() { X = start.X, Y = i });
                }
            }
            else
            {
                int min = Math.Min(start.X, end.X)+1;
                int max = Math.Max(start.X, end.X);
        
                for (int i = min; i <= max; i++)
                {
                    line.Add(new Vector2i() { X = i, Y = start.Y });
                }
            }
        ...
        //
        //private Vector2i FindEndPoint(Vector2i start, List<Vector2i> moves)
        //{
        //    foreach (var move in moves)
        //    {
        //        start = start.Add(move);
        //    }
        //    return start;
        //}

        private void DrawMap(List<Vector2i> map)
        {
            int up = map.Min(i => i.Y);
            int down = map.Max(i => i.Y);
            int left = map.Min(i => i.X);
            int right = map.Max(i => i.X);

            int width = right - left;
            int height = down - up;
            Vector2i offset = new Vector2i() {X = Math.Abs(left), Y = Math.Abs(up)};

            //char[,] chars = new char[width,height];
            Console.Clear();
            foreach (var v in map)
            {
                Vector2i offsetV = v.Add(offset);
                Console.SetCursorPosition(offsetV.X, offsetV.Y);
                Console.Write('-');
            }
        }

        private void ParseInput(string input, out List<Vector2i> map, out List<Vector2i> intersections)
        {
            map = new List<Vector2i>();
            intersections = new List<Vector2i>();
            
            void AddStep(Vector2i vector, List<Vector2i> map_, List<Vector2i> intersections_)
            {
                Console.WriteLine($"Add {vector}");
                if(map_.Contains(vector))
                {
                    intersections_.Add(vector);
                }
                else
                {
                    map_.Add(vector);
            ...
            var wires = input.Split(new string[] { "\r\n" }, StringSplitOptions.None);
            foreach(var wire in wires)
            {
                //Reset the position
                int previousX = 0;
                ...
                var lines = wire.Split(new string[] { "," }, StringSplitOptions.None);
                foreach (var line in lines)
                {
                    ...
                    {
                        case 'R':
                            while (x < previousX + value)
                            {
                                AddStep(new Vector2i() { X = x, Y = y }, map, intersections);
                                x++;
                            }
                            break;
                        case 'L':
